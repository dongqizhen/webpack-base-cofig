...
class Bannerheader extends React.Component {
	...
	render() {
		...
		for (const key in this.state.json) {
			const str = (
				<li
					className={`listItem-${n++}`}
					key={key}
					onMouseEnter={this.mouseEnterEvent.bind(this, n)}
				...
				>
					...
					<div className="moudel">
						{this.state.json[key].map((value, key) => <span key={key}> {value}</span>)}
					</div>

					{this.state.stores.currentIndex == n &&
					this.state.stores.BannerHideBoxIsShow && <BannerHideContainer />}
				</li>
			);
...
class Header extends React.Component {
	...
	handleClick(e) {
		this.state.inputValue == '' ? e.preventDefault() : '';
	}
			...
			<Router basename="/">
				<div className="header">
					...
					<div className="search clearfix">
						...
						<div className="searchContainer">
							<div className="seach_btn">
								...
								<input
									type="text"
									placeholder="请输入搜索内容，如飞利浦血压仪等"
									onChange={this.inputValueChangeEvent.bind(this)}
								/>
								<Link
									to={{ pathname: '/search', search: `?value=${this.state.inputValue}` }}
									target="_blank"
					...
					{/**
                         ...
                         */}
					{(this.props.bannerHeaderStyle == undefined || this.props.bannerHeaderStyle) && (
...
export { Header, Bannerheader, BannerHideContainer };
